...
public class Change_Password extends javax.swing.JFrame {
    ...
    public Change_Password() {
        initComponents();
        con=Connect.connect();
        id.setText(Login.id.getPassword().toString());
...
this.dispose();
    ...
    private void confirmKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_confirmKeyReleased
        if(confirm.getPassword().toString().equals("")){
         confirmed.setVisible(false);   
        }
        else if(confirm.getPassword().equals(new_pass.getPassword())){
...
if(old_pass.getPassword().equals("")||new_pass.getPassword().equals("")||confirm.getPassword().equals("")){
JOptionPane.showMessageDialog(null,"Complete Your Inforamtion","Missing Information",2);
  }     
   else {
    if(check_pass()){
        
        String sql = "update users set PASSWORD = '"+new_pass.getPassword().toString()+"' where ID='"+id.getText()+"' ";
        try {
    ...
    private void new_passKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_new_passKeyReleased
        if(new_pass.getPassword().toString().length()==0){
         we_st.setVisible(false);   
        }
        else if(new_pass.getPassword().toString().length()<6){
    ...
    private boolean check_pass(){
        ...
        try {
         ...
         if(res.next()){
             if(res.getString("PASSWORD").equals(old_pass.getPassword().toString())){
                 if(!(new_pass.getPassword().toString().length() < 6)){
                check=true;
                 } 
             }
         }
